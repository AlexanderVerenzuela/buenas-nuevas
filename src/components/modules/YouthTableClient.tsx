...
import { deleteYouth } from "@/actions/youth"
...
export function YouthTableClient({ initialData }: { initialData: any[] }) {
              ...
              return (
                <TableRow key={youth.id}>
                  <TableCell className="font-medium text-foreground">
                    <Link href={`/youth/${youth.id}`} className="hover:underline hover:text-primary transition-colors">
                      {youth.firstName} {youth.lastName}
                    </Link>
                    {youth.needsFollowUp && (
                      <span className="ml-2 inline-block h-2 w-2 rounded-full bg-red-600" title="Necesita Seguimiento" />
                    )}
                  </TableCell>
                  <TableCell>
