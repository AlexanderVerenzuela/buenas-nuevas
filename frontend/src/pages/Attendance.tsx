    ...
    <div className="space-y-6">
      <div className="flex items-center gap-6 p-6 border rounded-xl bg-card shadow-sm relative overflow-hidden">
        ...
        {getFirstPhoto(meeting.photoUrl) && (
          <div 
            onClick={() => setZoomedPhoto(getFirstPhoto(meeting.photoUrl))}
            className="relative z-10 w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden shadow-lg group cursor-pointer border-2 border-white/10 hover:border-white/30 transition-all flex-shrink-0"
          >
            <img src={getImageUrl(getFirstPhoto(meeting.photoUrl))} alt={meeting.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
